...
$('audio[controls]').before(function () {
  ...
  var addTitle = function (song_title) {
    var title_wrapper = document.createElement('div');
    $(title_wrapper).html(song_title);
    $(title_wrapper).addClass('btn btn-default  col-sm-3');
    $(player).append(title_wrapper);
  }; // addTitle
  ...
  var fillPlayerBox = function (title) {
    //addData();
    addPlayer(title);
  ...
  fillPlayerBox($(song).data('infoTitle'));
